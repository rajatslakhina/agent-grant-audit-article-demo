...
public enum DemoEngine {
    ...
    public static func present(
        ...
        let radius = BlastRadius.measure(folders: status.permittedFolders, projects: projects, now: now)

        let headline: String
        if status.allowsAllAgents {
            headline = "\(status.permittedFolders.count) folders listed. All \(projects.count) projects reachable."
        } else {
        ...
        return DemoPresentation(
            ...
            listedGrantCount: status.permittedFolders.count,
            refusal: nil
        )
    }
}
