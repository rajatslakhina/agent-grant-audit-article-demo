...
public struct GrantAuditor: Sendable {
    ...
    public func audit(_ status: MCPServerStatus, now: Date) -> AuditReport {
        ...
        for agent in status.permittedAgents where !policy.approvedTeamIdentifiers.contains(agent.teamIdentifier) {
            findings.append(
                Finding(
                    ...
                    remediation: "Add \(agent.teamIdentifier) to approvedTeamIdentifiers in review, or revoke grant \(agent.id)."
                )
            )
        }

        for grant in status.permittedFolders {
            if grant.expiry.hasExpired(at: now) {
                findings.append(
                    Finding(
                        code: .grantAlreadyExpired,
                        severity: .info,
                        detail: "Grant \(grant.id) on \(grant.subtreeRoot) has expired and no longer admits anything.",
            ...
            if !policy.approvesRoot(of: grant.subtreeRoot) {
                findings.append(
                    Finding(
                        code: .grantOutsideApprovedRoots,
                        severity: .blocking,
                        detail: "Grant on \(grant.subtreeRoot) is not contained by any approved root. The service records it as a subtree root, so it reaches every project beneath it — code this policy never reviewed.",
                        remediation: "Revoke grant \(grant.id) and re-take it scoped to a repository root listed in approvedGrantRoots."
                    )
                )
            }
            ...
            case .never where !policy.permitsNonExpiringGrants:
                findings.append(
                    Finding(
                        ...
                        severity: .warning,
                        detail: "Grant on \(grant.subtreeRoot) never expires. It outlives the task that needed it, the branch it was taken for, and usually the person who clicked it.",
            ...
            case .at(let instant) where instant.timeIntervalSince(now) > policy.maximumGrantLifetime:
                findings.append(
                    Finding(
                        ...
                        severity: .warning,
                        detail: "Grant on \(grant.subtreeRoot) has \(Int(instant.timeIntervalSince(now) / 3600))h left, over the \(Int(policy.maximumGrantLifetime / 3600))h ceiling.",
