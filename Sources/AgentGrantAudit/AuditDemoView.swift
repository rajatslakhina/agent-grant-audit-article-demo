    ...
    private var blastRadiusCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if presentation.bypassActive {
                Text("permittedFolders is empty, so grant-based reachability is 0. That number is not a safety signal here — the bypass admits every process on the machine, and leaves nothing to revoke.")
                    .font(.footnote)
            } else if let radius = presentation.blastRadius {
                if let widest = radius.widestGrant {
                    Text("Widest live grant: \(widest.subtreeRoot.description)")
                        .font(.system(.footnote, design: .monospaced))
                }
