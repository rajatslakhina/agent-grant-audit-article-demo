...
The write-up this package was built for: **[Xcode 27's Agent Permissions: The Safest-Looking State Is the Most Dangerous](https://medium.com/@er.rajatlakhina/xcode-27s-agent-permissions-the-safest-looking-state-is-the-most-dangerous-eebf51101546)**.
...
One field here is **inferred, not observed**: the published `status --format json` shows only `id` and `subtreeRoot` per grant, with no expiry. `StatusDecoder` therefore reads a missing expiry as `.never`, which is the pessimistic reading — but it means that on a current build **every** grant decodes as permanent, and the shipped `permitsNonExpiringGrants: false` will warn on all of them, including one taken with *Allow for 24 Hours*. Set it to `true` until the field is confirmed to exist.
