...
A small Swift package that reads the local agent-authorisation state, grades it against a **policy you commit to your repository**, and returns an exit code a build phase can act on.

It exists to make one argument concrete: when a coding agent is handed the toolchain, the grant that admits it is a security artefact — and right now that artefact lives in a dialog box nobody reviews.

> Companion demo for the article: (added after publish)

---

## The finding the package is built around

An empty grant list is the most dangerous state this configuration can be in.

Two very different machines produce it. One has never trusted anything. The other has the machine-wide bypass switched on — which trusts every process on the box, and, because the bypass is not itself a grant, **leaves nothing in the grant list to revoke**. Read only the grants and both machines look identically, reassuringly clean.
...
`StatusDecoder` requires every field the verdict depends on. A document missing `allowAllAgents` is refused, not defaulted:
...
That has a real cost — the gate goes red the first time the vendor renames a key, on a machine that was never misconfigured. It is still the better failure. The alternative is a renamed key decoding to `false`, the bypass going unseen, and the gate reporting green, which is the failure that does damage because nothing about it looks like one.
...
|---|---|
| `~/Developer/checkout-ios`, recursive, 12h | 2 |
| `~/Developer`, recursive, permanent | 9 |
| bypass on, zero grants listed | all 12 |
...
Apple does not document the JSON emitted by `xcrun mcp-server status --format json`. The field names here (`enabled`, `allowAllAgents`, `agents[].signingIdentifier`, `agents[].teamIdentifier`, `folderGrants[].path`) follow the fields reported in public write-ups of the tool. Treat the schema as a moving target — which is the argument for pinning your expectations in code you own and failing closed when they stop matching, rather than a reason not to check at all.
